hromSizes - script to grab chrom.sizes from UCSC via either of:
#	mysql, wget or ftp
...
if [ -n "${WGET}" ]; then
	...
	cat ${tmpResult} | grep -v 'chrU' | grep -v 'random' | grep -v 'chr6_' | grep -v 'chr17_' | grep -v 'chr4_' | sed 's/chr//' | sed 's/X/23/' | sed 's/Y/24/'  | sed 's/M/25/'
	rm -f "${tmpResult}"
	DONE=1
    fi
fi
...
bye\n" > ${tmpFtpRsp}
	...
	cat ${tmpResult} | zcat | cut -f1,2 | sort -k2nr | grep -v 'chrU' | grep -v 'random' | grep -v 'chr6_' | grep -v 'chr17_' | grep -v 'chr4_' | sed 's/chr//' | sed 's/X/23/' | sed 's/Y/24/' | sed 's/M/25/'
	rm -f "${tmpFtpRsp}" "${tmpResult}"
	DONE=1
    fi
fi
